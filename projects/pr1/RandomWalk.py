...
realStateValues = np.array([float(i) / N_STATES for i in xrange(N_STATES + 2)])
realStateValues[-1] = 0.0
...
class ValueFunction:
    ...
    def __init__(self, lambda_, stepSize):
...
class OffLineLambdaReturn(ValueFunction):
    def __init__(self, lambda_, stepSize):
        ValueFunction.__init__(self, lambda_, stepSize)
    ...
    def lambdaReturnFromTime(self, time):
        ...
        for n in range(1, self.T - time):
            ...
            lambdaPower *= self.lambda_
            if lambdaPower < self.lambda_Truncate:
                # If the power of lambda has been too small, discard all the following sequences
                break
        returns *= 1 - self.lambda_
        if lambdaPower >= self.lambda_Truncate:
...
class TrueOnlineTemporalDifferenceLambda(ValueFunction):
    ...
    def learn(self, state, reward):
        ...
        dutch = 1 - self.stepSize * self.lambda_ * self.eligibility[self.lastState]
        self.eligibility *= self.lambda_
    ...
    for run in range(runs):
        for lambdaIndex, lambda_ in zip(range(len(lambdas)), lambdas):
            for alphaIndex, alpha in zip(range(len(alphas[lambdaIndex])), alphas[lambdaIndex]):
                valueFunction = valueFunctionGenerator(lambda_, alpha)
                for episode in range(episodes):
                    print('run:', run, 'lambda:', lambda_, 'alpha:', alpha, 'episode:', episode)
