...
while avg_rewards < 200:
    ...
    while not done and steps < 500:
        ...
        if steps == 500 and not done:
            reward -= 1000
        elif left_leg + right_leg != 2 and done:
            reward -= 1000
        if all([abs(vx) < 0.2, abs(angle) < 0.1, abs(v_angle) < 0.1, vy > -0.3, vy < -0.2]):
            good = True
            reward += 1
        else:
            good = False
    ...
    for index in samples:
        ...
        Q_s = nn.predict(state.reshape(1, 8))[0]
        Q_s_prime = target_nn.predict(state_prime.reshape(1, 8))[0]
    ...
    output = nn.fit(np.array(x), np.array(y), verbose=False)
    loss = sum(output.history.get('loss', []))

    if steps_since_update > target_update_steps or not double:
        steps_since_update = 0
        target_nn.set_weights(nn.get_weights())


nn.save('/tmp/solved-model')
